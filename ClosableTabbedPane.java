...
public class ClosableTabbedPane extends JTabbedPane {
 ...
 LinkedList<AgentInterface> map = new LinkedList<AgentInterface>();
 
 public void addTab(String title, AgentInterface component) {
  if( !tabAlreadyExists(title) ) {
   super.addTab(title + "  ", (Component) component);
   map.add( component );
  }
 }
 
 private boolean tabAlreadyExists(String title) {
     int numTabs = this.getTabCount();
  for( int i = 0 ; i < numTabs ; i++ ) {
   if( getTabTitleAt(i).equals(title)) {
    return true;
   }
  }
  return false;
 ...
 public boolean renameTab( String oldTitle, String newTitle ) {
  //int index = super.indexOfTab(oldTitle);
     int numTabs = this.getTabCount();
  for( int i = 0 ; i < numTabs ; i++ ) {
   if( getTabTitleAt(i).equals(oldTitle)) {
  ...
  public void mouseReleased(MouseEvent me) {
   if (closeUnderMouse(me.getX(), me.getY())) {
    ...
    if (isToCloseTab && selectedTab > 0) {
      AgentInterface mapResult = map.get( selectedTab - 1 );
      try{
        mapResult.noLongerVisible();
        map.remove( mapResult );
      }catch(Exception exc){System.out.println("Could not cast");};
