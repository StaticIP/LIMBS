...
public class GoalWidget extends ComponentWidget {
  ...
  public GoalWidget(Proposition goal) {
    this.prop = goal;
    this.shape = new Polygon();
    this.shape.addPoint(x-width/2, y);
    this.shape.addPoint(x, y-width/2);
    this.shape.addPoint(x+width/2, y);
    this.shape.addPoint(x, y+width/2);
    this.prop.getSimulation().addGoalWidget(this);
  }
  
  public void delete(){
    this.prop.delete();
    this.prop.getSimulation().removeGoalWidget(this);
  }
  ...
  public void setCentre(final double x, final double y) {
    ...
    this.shape = new Polygon();
    this.shape.addPoint(this.x-width/2, this.y);
  ...
  public ComponentWidget copy() {
    Proposition copiedGoal = (Proposition) this.prop.copy();
    ComponentWidget copy = new GoalWidget(copiedGoal);
